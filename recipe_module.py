...
'''
    Diccionario para filtrar resultados inútiles
'''

garbage_dict={'sea':None, 'old':None,'cloves':None,'clove':None,'cup':None, \
         'cups':None,'':None,'mixture':None,'minutes':None,'crosswise':None, \
         'piece':None,'pieces':None,'sauce':None,'handfull':None,'Handfull':None,\
         'fork':None }
...
def get_recipe(plate, spacy):
    ...
    if contents['count'] !=0:
        best_hit,name=get_best_hit(plate,contents) #Devuelve una lista con los ingredientes sin procesar  y el nombre del plato
        ...
        result= {"state":"Success", "Plate:":name,"recipe":result}     #El parámetro del diccionario devuelto "state" indicará si la operación tuvo éxito o no.
...
def get_best_hit(plate,contentJSON):
    ...
    for x in contentJSON["hits"] :
        ...
        similarity=get_label_similarity(plate,title)
        if(similarity>max):
            max=similarity
            best_hit=x
            name=title
...
def parse_recipe(input, spacy):
    ...
    for x in input:
        ...
        try:
            ...
            if(d["name"]!=None and not d['name'] in garbage_dict):
                result.append(d)
